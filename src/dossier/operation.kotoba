(ns dossier.operation
  "OperationActor — one intake/relationship/disclosure/correction operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Dossier-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the DisclosureGovernor (:govern) and the rollout
  phase gate (:decide) before anything commits to the SSoT or is disclosed.
  ...
  (:require [langgraph.graph :as g]
            ...
            [dossier.llm :as llm]
            [dossier.policy :as policy]
            [dossier.phase :as phase]
...
(defn build
  "Compiles an OperationActor graph bound to `store` (any `dossier.store/Store`).
  opts:
    :advisor      — a `dossier.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Dossier-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (llm/-advise advisor store request)]
            {:proposal p :audit [(llm/trace request p)]})))

      ;; DisclosureGovernor — independent censor (separate system than the LLM).
      (g/add-node :govern
               ...
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:correction? verdict) :data-subject-dispute
                                          (:high-stakes? verdict) :high-stakes
                                          :else :low-confidence))
                        :phase ph
      ...
      ;; (compliance officer) resumes with :approval. Then route commit/hold.
      ...
      ;; Hold — write the rejection to the ledger; no SSoT mutation, no disclosure.
      (g/add-node :hold
        (fn [{:keys [audit]}]
