...
実配線は完了(ADR addendum 4 参照)。

## 11. 実データソース: UK Companies House(ADR addendum 5)

R0はこれまで全て MemStore のデモデータだったが、`dossier.companies-house`
(`.clj`、http-kit + jsonista、`langchain.jvm/jvm-http-fn` と同じ host-fn
注入型)が最初の**実**ライブデータ経路を提供する。`dossier.live-store/
LiveGbrStore` が `Store` プロトコルのデコレータとして、既存の
local(Mem/Datomic)Store を包み、`company`/`company-by-name`/
`officials-of` の3メソッドだけを「local に無ければ実 Companies House API
を試す」に拡張する(local に既にある回答は絶対に上書きしない — 追加のみ、
変更なし)。

- 認証: `COMPANIES_HOUSE_API_KEY` 環境変数(Basic auth、パスワード空文字)。
  無ければ `configured?` が false を返し、`live-store` は無条件で
  undecorated な local store と同じ挙動(クラッシュしない、部分データを
  捏造しない)。
- スコープは正直に限定: `company-by-name`(`/search/companies` の完全一致)
  + 既知の法人IDに対する `officials-of`(`/company/{number}/officers`)のみ。
  `official-by-name`(人物名からグローバル検索)は**未実装**——Companies
  House の `/search/officers` は所属法人を返さず、候補ごとに
  `/officers/{id}/appointments` への追加呼び出しが要るため、中途半端に
  作らず明示的に見送った。つまり `:disclosure/screen-name`(パイロット5
  repoが実際に呼ぶop)はまだライブデータの恩恵を受けない。
- テストは全てオフライン(`{:path :query} -> レスポンス` の偽 fetch-fn を
  注入、実キー・実通信不要)。**このパスは実 Companies House API に対して
