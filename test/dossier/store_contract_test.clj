...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      ...
      (is (= "969500DEMO0JPN00001A" (:lei (store/company s "co-100")))
          ":lei (dossier.gleif's live seam attribute) round-trips on both backends")
      (is (nil? (:lei (store/company s "co-200")))
          "a company with no seeded LEI round-trips as nil, never a fabricated value")
